...
1. ✅ **Python 3.8+** and **dlt** installed in your virtual environment
2. ✅ **Docker Desktop** installed and running
3. ✅ **Working dlt pipeline** from Module 2 (`pipeline_with_staging.py`)
4. ✅ **Snowflake account** with `ACCOUNTADMIN` privileges (for SPCS setup)
5. ✅ **Virtual environment activated** for this project
...
Your Local Machine
   ↓
GitHub API → dlt → S3 → Snowflake
```

**New (Module 3):**
```
Snowflake SPCS Container (runs inside Snowflake)
   ↓
GitHub API → dlt → S3 → Snowflake
   ↑
Triggered by Snowflake Task (scheduled)
```
...
### 1.1 Create a Dedicated Pipeline File

Create `load_github_to_snowflake.py` (standalone version for SPCS):
...
```bash
python load_github_to_snowflake.py
```
...
CMD ["python", "-u", "load_github_to_snowflake.py"]
```
...
**Important:** Replace `kgiotue-wn98412` with your Snowflake account identifier.
...
**Replace `kgiotue-wn98412`** with your Snowflake account identifier.
...
**Option B: Via SQL**
```sql
PUT file:///Users/your-username/Desktop/Snowflake/load-github.yaml 
...
USE SCHEMA GITHUB_SPCS;

-- View all tables
SHOW TABLES IN SCHEMA GITHUB_SPCS;

-- Check repos data
SELECT COUNT(*) AS total_repos FROM GITHUB_SPCS.REPOS;
...
SELECT 
    ...
    STARGAZERS_COUNT,
    _DLT_LOAD_ID
FROM GITHUB_SPCS.REPOS
ORDER BY STARGAZERS_COUNT DESC
LIMIT 10;

-- Check contributors data
SELECT COUNT(*) AS total_contributors FROM GITHUB_SPCS.CONTRIBUTORS;

SELECT 
    LOGIN,
    CONTRIBUTIONS,
    _DLT_LOAD_ID
FROM GITHUB_SPCS.CONTRIBUTORS
...
You should see files in the `snowflake-demo/github_spcs/` folder.
...
**Check:**
- Is your account identifier correct? (`kgiotue-wn98412`)
...
Then execute the job again.

---

## Cost Optimization

### Compute Pool Pricing

| Instance Family | Credits/Hour | Cost vs Warehouse |
|-----------------|--------------|-------------------|
| CPU_X64_XS | ~0.025 | ~6% of XS warehouse |
| CPU_X64_S | ~0.05 | ~12% of S warehouse |
| XS Warehouse | ~1.0 | Baseline |

**Recommendation:** Use `CPU_X64_XS` for most dlt pipelines.
...
✅ **Cost Efficient**: SPCS containers are ~94% cheaper than warehouses  
...
- ✅ Add **incremental loading** for repos resource
- ✅ Implement **error handling** and **retry logic**
- ✅ Set up **monitoring** with Snowflake alerts
- ✅ Create **downstream transformations** triggered by pipeline completion
- ✅ Explore **dbt integration** for transformation layer

---

**Module 3 Complete!** You now have a production-grade, fully containerized dlt pipeline running inside Snowflake. 🎉
