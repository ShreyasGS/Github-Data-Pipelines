...
3. **Working dlt pipeline** from Module 2 (`github_api_pipeline.py`)
...
> Run these BEFORE starting if you've done this module before.
...
-- Drop SPCS infrastructure so demo shows fresh creation
DROP COMPUTE POOL IF EXISTS CP_DLT_PIPELINE;
DROP SCHEMA IF EXISTS DLT_DATA.GITHUB_DLT_SPCS CASCADE;
DROP EXTERNAL ACCESS INTEGRATION IF EXISTS EAI_GITHUB_DLT;
```
